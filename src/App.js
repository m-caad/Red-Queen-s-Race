...
// New Block of code And first code is also running but comment it..
...
function App() {
	...
	let adjustBackgroundPlayback = function () {
		const redQueen_alice_play_backrate =
			redQueen_alice.getAnimation().playbackRate;
		if (redQueen_alice_play_backrate < 0.8) {
			sceneries.forEach(function ({ getAnimation }) {
				getAnimation().updatePlaybackRate(
					(redQueen_alice_play_backrate / 2) * -1
				);
			});
	...
	useEffect(() => {
		...
		setInterval(function () {
			...
			if (redQueen_alice_animation.playbackRate > 0.4) {
				const newPlayback = redQueen_alice_animation.playbackRate * 0.9;
		...
		<div onClick={goFaster}>
			<div className='wrapper'>
				...
				<div className='scenery' id='foreground1' ref={foreground1Movement.ref}>
					<img id='palm3' src={palm3_small} srcSet={`${palm3} 2x`} alt='palm3' />
				</div>

				<div className='scenery' id='foreground2' ref={foreground2Movement.ref}>
					<img id='bush' src={bush_small} srcSet={`${bush} 2x`} alt='bush' />
				...
				<div className='scenery' id='background1' ref={background1Movement.ref}>
					...
					<img id='palm1' src={palm1_small} srcSet={`${palm1} 2x`} alt='palm1' />
				</div>
				<div className='scenery' id='background2' ref={background2Movement.ref}>
					...
					<img id='palm2' src={palm2_small} srcSet={`${palm2} 2x`} alt='palm2' />
				</div>
			</div>
		</div>
	);
